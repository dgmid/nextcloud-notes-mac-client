		...
		//@exclude
		{
			...
			[
				...
				},
				{
					...
					accelerator: 'Cmd+J',
					click (item, focusedWindow) { if(focusedWindow) app.emit('dev-config') }
				},
				{
					label: 'Open database.json',
					accelerator: 'Cmd+Alt+J',
					click (item, focusedWindow) { if(focusedWindow) app.emit('dev-database') }
				},
				{
					type: 'separator'
				},
				{
					label: 'Open Main Log',
					accelerator: 'Cmd+Ctrl+Alt+1',
					click (item, focusedWindow) { if(focusedWindow) app.emit('dev-main') }
				},
				{
					label: 'Open Render Log',
					accelerator: 'Cmd+Ctrl+Alt+2',
					click (item, focusedWindow) { if(focusedWindow) app.emit('dev-render') }
				}
